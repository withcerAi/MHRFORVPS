...
class H2Transport:
    ...
    def __init__(self, connect_host: str, sni_host: str,
                 ...
                 sni_start_index: int = 0,
                 stream_timeout: float = 75.0,
        ...
        self._sni_idx: int = int(sni_start_index or 0) % max(1, len(self._sni_hosts))
        self._stream_timeout: float = max(1.0, float(stream_timeout or 75.0))
        self._connect_timeout: float = max(1.0, float(connect_timeout or 15.0))
    ...
    async def request(self, method: str, path: str, host: str,
        ...
        effective_timeout = self._stream_timeout if timeout is None else float(timeout)

        for _ in range(follow_redirects + 1):
            status, resp_headers, resp_body = await self._single_request(
                method, path, host, headers, body, effective_timeout,
            )
