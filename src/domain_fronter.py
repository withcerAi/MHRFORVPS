...
class _IgnoreSslEofWarning(logging.Filter):
    def filter(self, record):
        return "returning true from eof_received() has no effect when using ssl" not in record.getMessage()

logging.getLogger("asyncio").addFilter(_IgnoreSslEofWarning())
...
class DomainFronter:
    ...
    _H2_FAILURE_COOLDOWN = 15.0
    _H2_FAILURE_THRESHOLD = 5
    ...
    def __init__(self, config: dict):
        ...
        self._script_ids = [
            str(s).strip()
            for s in self._script_ids
            if s is not None and str(s).strip()
        ]
        if not self._script_ids:
            raise ValueError(
                "Missing script_id/script_ids in config. "
                "Set config.json -> script_id or script_ids."
            )

        self._script_idx = 0
        self.script_id = self._script_ids[0]  # backward compat / logging
        try:
            stats.configure_scripts(
                self._script_ids,
                config.get("script_id_names", {}),
                int(config.get("script_quota_limit", 20000)),
            )
        except Exception:
            pass
        ...
        # PATCH_VIDEO_PRIORITY_DOMAIN_FRONTER
        self._video_priority_retry_attempts = self._cfg_int(
            config, "video_priority_retry_attempts", 4, minimum=1
        )
        self._video_priority_retry_delay_ms = self._cfg_int(
            config, "video_priority_retry_delay_ms", 80, minimum=0
        )
        self._video_priority_parallel_relay = self._cfg_int(
            config, "video_priority_parallel_relay", self._parallel_relay, minimum=1
        )
        self._video_priority_parallel_relay = max(
        ...
        # Dedicated timeout for synthetic Range probes used by parallel download.
        # This must be independent from generic relay timeout because large
        # initial probes can take longer through Apps Script/H1 under load.
        self._range_probe_timeout = self._cfg_float(
            config, "range_probe_timeout", self._relay_timeout, minimum=5.0,
        )
        # H2 per-stream timeout is configurable from config.json.
        # If not set, it follows relay_timeout so H2 does not die at the old hardcoded 25s/75s.
        self._h2_stream_timeout = self._cfg_float(
            config, "h2_stream_timeout", self._relay_timeout, minimum=5.0,
        )
        self._h2_connect_timeout = self._cfg_float(
            config, "h2_connect_timeout", config.get("tls_connect_timeout", TLS_CONNECT_TIMEOUT), minimum=1.0,
        )
        ...
        self._batch_permanent_disable = not bool(config.get("enable_batch", True))
        self._batch_enabled = not self._batch_permanent_disable
        self._sub_batch_enabled = bool(config.get("enable_sub_batch", True))
        ...
        # Front-IP diagnostics: helps identify when the Google front IP/path
        # itself is unhealthy, instead of blaming Apps Script or the origin.
        self._front_ip_timeout_window: list[float] = []
        self._front_ip_last_warning_at = 0.0
        self._front_ip_timeout_window_seconds = self._cfg_float(
            config, "front_ip_diagnostic_window_seconds", 120.0, minimum=10.0,
        )
        self._front_ip_timeout_threshold = self._cfg_int(
            config, "front_ip_diagnostic_timeout_threshold", 5, minimum=1,
        )
                ...
                if n_conns > 0:
                    self._h2_pool = [
                        H2Transport(
                            ...
                            sni_start_index=i,
                            stream_timeout=self._h2_stream_timeout,
                            connect_timeout=self._h2_connect_timeout,
                        )
                        for i in range(n_conns)
                    ]

                    self._h2 = self._h2_pool[0]
                    ...
                    log.info("HTTP/2 multiplexing disabled by config")
    ...
    def _h2_available(self) -> bool:
        ...
        for t in self._h2_pool:
            try:
                if bool(getattr(t, "is_connected")):
                    return True
            except Exception:
                pass
            try:
                if bool(getattr(t, "_connected", False)):
                    return True
            except Exception:
                pass
        return False
    ...
    def _pick_h2(self):
        ...
        for i in range(n):
            t = self._h2_pool[(self._h2_pool_idx + i) % n]
        ...
        return self._h2_pool[0]

    def _record_front_ip_timeout(self, *, path: str, host: str = "", url: str = "", exc: Exception | None = None) -> None:
        """
        Track repeated relay/H2/H1 timeouts that strongly suggest the configured
        Google front IP path is unhealthy.

        This does not prove the IP is always the only problem, but when many
        unrelated hosts fail through the same connect_host, the front IP/path is
        the most likely bottleneck.
        """
        now = time.time()
        window = float(getattr(self, "_front_ip_timeout_window_seconds", 120.0) or 120.0)
        threshold = int(getattr(self, "_front_ip_timeout_threshold", 5) or 5)
        cooldown = float(getattr(self, "_front_ip_warning_cooldown", 30.0) or 30.0)

        events = getattr(self, "_front_ip_timeout_window", [])
        events.append(now)
        cutoff = now - window
        events = [t for t in events if t >= cutoff]
        self._front_ip_timeout_window = events

        count = len(events)
        if count < threshold:
            log.warning(
                "FRONT-IP signal: timeout via %s front_ip=%s sni_pool=%s host=%s count=%d/%d window=%.0fs detail=%s",
                path,
                getattr(self, "connect_host", "-"),
                ",".join(getattr(self, "_sni_hosts", []) or []),
                host or "-",
                count,
                threshold,
                window,
        ...
        last_warn = float(getattr(self, "_front_ip_last_warning_at", 0.0) or 0.0)
        if now - last_warn < cooldown:
            return

        self._front_ip_last_warning_at = now

        log.error(
            "FRONT-IP UNHEALTHY: repeated relay timeouts through google_ip=%s. "
            "Likely bad/filtered/lossy path to the configured Google front IP, not necessarily origin=%s. "
            "timeouts=%d in %.0fs, path=%s, sni_pool=%s, sample_url=%s. "
            "Action: test/change google_ip or lower H2/prefetch/fanout pressure.",
            getattr(self, "connect_host", "-"),
            host or "-",
            count,
            window,
    ...
    def _record_front_ip_success(self) -> None:
        # Decay the suspicion quickly when successful relay traffic returns.
        events = getattr(self, "_front_ip_timeout_window", [])
        if events:
            self._front_ip_timeout_window = events[-max(0, len(events) // 2):]


    def _record_h2_success(self) -> None:
        self._h2_failure_streak = 0
        try:
            self._record_front_ip_success()
        except Exception:
            pass
    ...
    def _available_script_ids(self) -> list[str]:
        """Return script IDs that are not currently blacklisted."""
        self._prune_blacklist()
        return [
            sid for sid in self._script_ids
            if not self._is_sid_blacklisted(sid)
        ]

    def _require_available_script_id(self, preferred: str | None = None) -> str:
        """Hard-enforce script blacklist."""
        available = self._available_script_ids()

        if preferred and preferred in available:
            return preferred

        if available:
            sid = available[self._script_idx % len(available)]
            self._script_idx += 1
            return sid

        raise RuntimeError("All Apps Script deployments are temporarily disabled/blacklisted")
    ...
    def _is_sid_blacklisted(self, sid: str) -> bool:
        ...
        if until:
            ...
            try:
                self._script_strikes.pop(sid, None)
                self._script_last_reason.pop(sid, None)
            except Exception:
                pass
            log.info("RE-ENABLED script %s", _mask_sid(sid))

        return False

    def _record_script_failure(self, sid: str, reason: str = "") -> None:
        """Count script failures and disable only after repeated strikes."""
        if not sid:
            return
        ...
        strikes = int(getattr(self, "_script_strikes", {}).get(sid, 0) or 0) + 1
        try:
            self._script_strikes[sid] = strikes
            self._script_last_reason[sid] = str(reason or "")
        except Exception:
            pass
        ...
        if strikes < limit:
            log.warning(
                "Script warning %s strike=%s/%s reason=%s",
                _mask_sid(sid),
                strikes,
                limit,
                reason or "-",
    ...
    def _blacklist_sid(self, sid: str, reason: str = "") -> None:
        """Blacklist a script ID, but do not disable it for one transient timeout."""
        if getattr(self, "_h2_rebuilding", False):
            log.debug("Skip blacklist during H2 rebuild: %s", _mask_sid(sid))
            return
        ...
        hard_failure = (
            ...
            or "limit" in reason_l
            or "bad_status_429" in reason_l
            or "bad_status_500" in reason_l
            or "bad_status_502" in reason_l
            or "bad_status_503" in reason_l
            or "bad_status_504" in reason_l
        )

        transient = (
            "timeout" in reason_l
            or "connectionerror" in reason_l
            or "connection" in reason_l
            or "eof" in reason_l
            or "reset" in reason_l
            or "brokenpipe" in reason_l
        )

        # Timeout/connection failures need 3 strikes before a real 10-minute disable.
        if transient and not hard_failure:
            strikes = self._sid_failure_counts.get(sid, 0) + 1
            self._sid_failure_counts[sid] = strikes

            try:
                stats.add_google_request(
                    ...
                    status_code="WARN",
                    script_id=sid,
                    response_text=f"script warning strike={strikes}/3 reason={reason_s}",
                )
            except Exception:
                pass

            log.warning(
                "Script warning %s strike=%d/3 reason=%s",
                _mask_sid(sid),
                strikes,
                reason_s,
            )
            ...
            self._sid_failure_counts[sid] = 0

        ttl = int(self._blacklist_ttl)
        self._sid_blacklist[sid] = time.time() + ttl

        event = {
            "sid": _mask_sid(sid),
            "reason": reason_s,
            "disabled_for_s": ttl,
            "until": int(self._sid_blacklist[sid]),
        }
        self._sid_last_blacklist_events.append(event)
        self._sid_last_blacklist_events = self._sid_last_blacklist_events[-20:]
        ...
        try:
            stats.add_google_request(
                ...
                script_id=sid,
                response_text=f"script disabled for {ttl}s: {reason_s}",
            )
        except Exception:
            pass

        log.warning(
            "DISABLED script %s for %ds (%s)",
            _mask_sid(sid),
            ttl,
            reason_s,
        )
    ...
    @staticmethod
    def _raw_status(raw: bytes) -> int:
        try:
            first = (raw or b"").split(b"\r\n", 1)[0].decode(errors="replace")
            m = re.search(r"\s(\d{3})\s", first)
            return int(m.group(1)) if m else 0
        except Exception:
            return 0

    def _is_bad_relay_response(self, raw: bytes) -> bool:
    ...
    def _is_video_priority_payload(self, payload: dict) -> bool:
        u = str(payload.get("u", "") or "").lower()
        h = self._host_key(u)
        return (
            "googlevideo.com" in h
            or "youtube.com" in h
            or "ytimg.com" in h
    ...
    def _attempts_for_payload(self, payload: dict) -> int:
        if self._is_video_priority_payload(payload):
            return max(
                self._retry_attempts_for_payload(payload),
                int(getattr(self, "_video_priority_retry_attempts", 4) or 4),
            )
    ...
    async def _video_retry_sleep(self, payload: dict, attempt: int):
        if not self._is_video_priority_payload(payload):
            return
        delay = int(getattr(self, "_video_priority_retry_delay_ms", 80) or 0)
        if delay > 0:
            await asyncio.sleep((delay / 1000.0) * max(1, attempt + 1))
    ...
    async def _relay_payload_h1(self, payload: dict, timeout_override: float | None = None) -> bytes:
        attempts = self._attempts_for_payload(payload)
        effective_timeout = float(timeout_override or self._relay_timeout)
        async with self._semaphore:
            for attempt in range(attempts):
                try:
                    return await asyncio.wait_for(
                        self._relay_single(payload), timeout=effective_timeout,
                    )
    ...
    async def _range_probe(self, url: str, headers: dict, start_off: int,
        ...
        for attempt in range(max_tries):
            try:
                last_raw = await self._relay_payload_h1(
                    probe_payload,
                    timeout_override=getattr(self, "_range_probe_timeout", self._relay_timeout),
                )
            except Exception as exc:
    ...
    def _record_google_stats(self, payload: dict, sent: int, received: int, script_id: str = "", error: bool = False) -> None:
        try:
            stats.add_google_request(
                ...
                received=received,
                error=error,
                script_id=script_id or self._script_id_for_key(self._host_key(payload.get("u"))),
            )
        except Exception:
            pass
    ...
    async def _h2_connect(self):
        """Connect all HTTP/2 transports in background."""
        if not self._h2_pool:
            return
        ...
        try:
            ...
            if connected > 0:
                ...
                log.warning(
                    "H2 connect failed (%s), using H1 pool fallback",
                    exc,
                )
    ...
    async def apply_h2_connections(self, n_conns: int):
        ...
        old_pool = list(getattr(self, "_h2_pool", []) or [])
        ...
        if n_conns <= 0:
            log.info("H2 disabled by runtime profile")
        ...
        try:
            from h2_transport import H2Transport, H2_AVAILABLE
            if not H2_AVAILABLE:
                log.warning("H2 requested but h2_transport is not available")
                return
    ...
    def export_runtime_state(self):
        """Return actual live DomainFronter runtime values."""
        h2_total = len(getattr(self, "_h2_pool", []) or [])
        h2_live = 0
        try:
            h2_live = sum(1 for h in self._h2_pool if getattr(h, "is_connected", False))
        except Exception:
            h2_live = 0
        ...
        blacklisted_detail = []
        try:
            for sid, until in self._sid_blacklist.items():
                if until > now:
                    blacklisted.append(sid)
                    blacklisted_detail.append({
                        "sid": _mask_sid(sid),
                        "state": "DISABLED",
                        "accepting_requests": False,
                        "remaining_s": int(max(0, until - now)),
                    })
        except Exception:
            blacklisted = []
            blacklisted_detail = []
        ...
        return {
            "front_connect_host": getattr(self, "connect_host", "-"),
            "front_sni_host": getattr(self, "sni_host", "-"),
            "front_http_host": getattr(self, "http_host", "-"),

            "script_count": len(getattr(self, "_script_ids", []) or []),
            "parallel_relay": getattr(self, "_parallel_relay", "-"),
            "relay_timeout": getattr(self, "_relay_timeout", "-"),
            "range_probe_timeout": getattr(self, "_range_probe_timeout", "-"),
            "tls_connect_timeout": getattr(self, "_tls_connect_timeout", "-"),
            "max_response_body_bytes": getattr(self, "_max_response_body_bytes", "-"),

            "h2_connections": h2_total,
            "h2_live_connections": h2_live,
            ...
            "batch_enabled": bool(getattr(self, "_batch_enabled", False)),
            "batch_permanent_disable": bool(getattr(self, "_batch_permanent_disable", False)),
            "enable_batch": not bool(getattr(self, "_batch_permanent_disable", False)),
            "enable_sub_batch": bool(getattr(self, "_sub_batch_enabled", False)),
            "batch_window_micro": getattr(self, "_batch_window_micro", "-"),
            "batch_window_macro": getattr(self, "_batch_window_macro", "-"),
            "batch_max": getattr(self, "_batch_max", "-"),

            "pool_max": getattr(self, "_pool_max", "-"),
            "pool_min_idle": getattr(self, "_pool_min_idle", "-"),
            "pool_idle": len(getattr(self, "_pool", []) or []),
            "blacklisted_scripts": len(blacklisted),
            "disabled_scripts": [
                {
                    "sid": _mask_sid(sid),
                    "remaining_s": int(max(0, until - now)),
                    "reason": getattr(self, "_script_last_reason", {}).get(sid, ""),
                }
                for sid, until in getattr(self, "_sid_blacklist", {}).items()
                if until > now
            ],
            "blacklisted_script_details": blacklisted_detail,
            "last_blacklist_events": list(getattr(self, "_sid_last_blacklist_events", []) or []),
    ...
    def apply_runtime_config(self, config: dict) -> None:
        """Apply live runtime profile values to DomainFronter."""
        try:
            self._relay_timeout = self._cfg_float(config, "relay_timeout", self._relay_timeout, minimum=1.0)
            self._range_probe_timeout = self._cfg_float(
                config,
                "range_probe_timeout",
                getattr(self, "_range_probe_timeout", self._relay_timeout),
                minimum=5.0,
            )

            # runtime h2 timeout apply
            self._h2_stream_timeout = self._cfg_float(
                config,
                "h2_stream_timeout",
                getattr(self, "_h2_stream_timeout", self._relay_timeout),
                minimum=5.0,
            )
            self._h2_connect_timeout = self._cfg_float(
                config,
                "h2_connect_timeout",
                getattr(self, "_h2_connect_timeout", self._tls_connect_timeout),
                minimum=1.0,
            )

            for _h2 in getattr(self, "_h2_pool", []) or []:
                if hasattr(_h2, "_stream_timeout"):
                    _h2._stream_timeout = self._h2_stream_timeout
                if hasattr(_h2, "_connect_timeout"):
                    _h2._connect_timeout = self._h2_connect_timeout
            self._tls_connect_timeout = self._cfg_float(config, "tls_connect_timeout", self._tls_connect_timeout, minimum=1.0)
            self._max_response_body_bytes = self._cfg_int(config, "max_response_body_bytes", self._max_response_body_bytes, minimum=1024)

            pr = self._cfg_int(config, "parallel_relay", self._parallel_relay, minimum=1)
            self._parallel_relay = max(1, min(pr, len(self._script_ids)))

            self._batch_window_micro = float(config.get("batch_window_micro", self._batch_window_micro))
            self._batch_window_macro = float(config.get("batch_window_macro", self._batch_window_macro))
            if self._batch_window_macro < self._batch_window_micro:
                self._batch_window_macro = self._batch_window_micro

            self._batch_max = self._cfg_int(config, "batch_max", self._batch_max, minimum=1)
            self._batch_permanent_disable = not bool(config.get("enable_batch", not self._batch_permanent_disable))
            self._batch_enabled = not self._batch_permanent_disable
            self._sub_batch_enabled = bool(config.get("enable_sub_batch", self._sub_batch_enabled))

            self._video_priority_retry_attempts = self._cfg_int(config, "video_priority_retry_attempts", self._video_priority_retry_attempts, minimum=1)
            self._video_priority_retry_delay_ms = self._cfg_int(config, "video_priority_retry_delay_ms", self._video_priority_retry_delay_ms, minimum=0)
    ...
    def _build_payload(self, method, url, headers, body):
        ...
        if headers:
            ...
            filt = {
                ...
                if str(k).lower() not in self._STRIP_HEADERS
            }
    ...
    async def _relay_with_retry(self, payload: dict) -> bytes:
        ...
        fanout_limit = self._fanout_limit_for_payload(payload)
        if (attempts > 1
                and fanout_limit > 1
            ...
            try:
                result = await asyncio.wait_for(
                    self._relay_fanout(payload),
                    timeout=getattr(self, "_h2_stream_timeout", self._relay_timeout),
                )
                self._record_h2_success()
                return result
            except Exception as e:
                self._record_h2_failure(e)
                if isinstance(e, (asyncio.TimeoutError, TimeoutError)) or type(e).__name__ == "TimeoutError":
                    self._record_front_ip_timeout(
                        path="H2 fanout-fallback",
                        host=self._host_key(payload.get("u")),
            ...
            for attempt in range(attempts):
                try:
                    ...
                    if isinstance(e, (asyncio.TimeoutError, TimeoutError)):
                        ...
                        log.warning(
                            "Relay timeout via H2 attempt=%s/%s timeout=%ss h2_stream_timeout=%s front_ip=%s host=%s url=%s",
                            attempt + 1,
                            attempts,
                            self._relay_timeout,
                            getattr(self, "_h2_stream_timeout", "-"),
                            getattr(self, "connect_host", "-"),
                            self._host_key(payload.get("u")),
            ...
            for attempt in range(attempts):
                try:
                    ...
                    if isinstance(e, (asyncio.TimeoutError, TimeoutError)):
                        ...
                        log.warning(
                            "Relay timeout via H1 attempt=%s/%s timeout=%ss front_ip=%s host=%s url=%s",
                            attempt + 1,
                            attempts,
                            self._relay_timeout,
                            getattr(self, "connect_host", "-"),
    ...
    async def _relay_fanout(self, payload: dict) -> bytes:
        ...
        old_limit = getattr(self, "_current_fanout_limit", None)
        self._current_fanout_limit = self._fanout_limit_for_payload(payload)
        try:
            sids = self._pick_fanout_sids(host_key)
        finally:
            if old_limit is None:
                try:
                    delattr(self, "_current_fanout_limit")
                except Exception:
                    pass
            else:
                self._current_fanout_limit = old_limit
        if len(sids) <= 1:
        ...
        try:
            while pending:
                ...
                for t in done:
                    ...
                    if exc is None:
                        candidate = t.result()
                        if not self._is_bad_relay_response(candidate):
                            winner_result = candidate
                            return winner_result
                        self._record_script_failure(sid, reason=f"bad_status_{self._raw_status(candidate)}")
                        winner_exc = RuntimeError(f"bad relay status {self._raw_status(candidate)}")
                        continue
                    # This racer failed — blacklist and keep waiting for others
                    reason = type(exc).__name__
                    msg = str(exc)

                    if isinstance(exc, asyncio.TimeoutError) or reason == "TimeoutError":
                        reason = "TimeoutError:h2_fanout_stream_or_apps_script"
                    elif "H2 stream" in msg and "timed out" in msg:
                        reason = "TimeoutError:h2_stream_no_response"
                    elif "ALPN" in msg:
                        reason = "TLS:alpn_failed"
                    elif "Connection lost" in msg or "Connection closed" in msg:
                        reason = "H2:connection_lost"
                    elif "Bad JSON" in msg or "No JSON" in msg:
                        reason = "AppsScript:bad_response"

                    log.warning(
                        "Relay failure detail path=H2 fanout sid=%s reason=%s detail=%s",
                        _mask_sid(sid),
                        reason,
                        msg[:220],
                    )
    ...
    async def _relay_single_h2(self, payload: dict) -> bytes:
        ...
        self._record_google_stats(
            payload,
            sent=len(json_body),
            received=len(body),
            script_id=self._script_id_for_key(self._host_key(payload.get("u"))),
            error=False,
        )
    ...
    async def _relay_single(self, payload: dict) -> bytes:
        ...
        try:
            ...
            sid_used = self._script_id_for_key(self._host_key(payload.get("u")))
            self._record_google_stats(
                payload,
                sent=len(json_body),
                received=len(resp_body),
                script_id=sid_used,
                error=False,
            )
    ...
    async def _relay_batch(self, payloads: list[dict]) -> list[bytes]:
        ...
        if self._h2_available():
            try:
                ...
                self._record_h2_success()
                try:
                    sid_used = self._script_id_for_key(
                        self._host_key(payloads[0].get("u")) if payloads else ""
                    )
                    stats.add_google_request(
                        self._host_key(payloads[0].get("u")) if payloads else "batch",
                        sent=len(json_body),
                        received=len(body),
                        error=False,
                        status_code=status,
        ...
        try:
            ...
            stats.add_google_request(
                ...
                sent=len(json_body),
                received=len(resp_body),
                error=(int(status or 0) >= 500),
                script_id=sid_used,
                count=max(1, len(payloads)),
                status=status,
                response_text=resp_body[:1500].decode(errors="replace"),
            )
        except Exception:
            pass
    ...
    def _parse_batch_body(self, resp_body: bytes,
        ...
        try:
            for payload, item in zip(payloads, items):
                sid_used = self._script_id_for_key(self._host_key(payload.get("u")))
                received = len(json.dumps(item).encode("utf-8")) if isinstance(item, dict) else len(str(item).encode("utf-8"))
                sent = len(json.dumps(payload).encode("utf-8"))
                self._record_google_stats(
                    payload,
                    sent=sent,
                    received=received,
                    script_id=sid_used,
                    error=bool(isinstance(item, dict) and item.get("e")),
                )
        except Exception:
            pass
    ...
    async def _read_http_response(self, reader: asyncio.StreamReader):
        ...
        if "chunked" in transfer_encoding:
            ...
            try:
                total = int(str(content_length).strip())
            except (TypeError, ValueError):
                raise RuntimeError(f"Invalid Content-Length from relay frontend: {content_length!r}")
    ...
    async def _read_chunked(self, reader, buf=b""):
        ...
        while True:
            ...
            try:
                # Supports chunk extensions like: 1a;foo=bar
                size = int(size_str.split(";", 1)[0], 16)
            except ValueError:
                break

            if size == 0:
                # Consume optional trailers / final CRLF so keep-alive stays clean.
